...
class _DxHomePageState extends State<DxHomePage> {
  ...
  Future<void> _restoreApiBaseUrl() async {
    ...
    apiController.text = prefs.getString(storageKey) ?? defaultApiBaseUrl;
    if (mounted) {
  ...
  Future<void> _download({required DownloadMode mode, String? quality, required String key}) async {
    ...
    try {
      final baseUrl = _normalizeBaseUrl(apiController.text);
      await _saveApiBaseUrl(baseUrl);
      final query = {'url': current.url, 'mode': mode.value, if (quality != null) 'quality': quality};
      ...
      if (body == null) {
        throw const DxException('Download failed before any file data was returned.');
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Saved ${file.path.split(Platform.pathSeparator).last}')));
    } catch (err) {
      if (!mounted) return;
      setState(() => error = _friendlyError(err));
    } finally {
  ...
  Future<void> _openLatestDownload() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result.message.isEmpty ? 'Could not open the file.' : result.message)));
  }
  ...
  Future<void> _copyLatestPath() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Download path copied.')));
  }
              ...
              : ListView(
                  ...
                  children: [
                    _panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('DX Downloader', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
                    ...
                    _panel(child: Column(children: [
                      TextField(controller: apiController, decoration: _input('Backend URL', 'http://10.0.2.2:3001')),
                      ...
                      TextField(
                        ...
                        decoration: _input('YouTube or Instagram link', 'https://www.youtube.com/watch?v=...'),
                      ...
                      SizedBox(
                        ...
                        child: FilledButton(
                          ...
                          style: FilledButton.styleFrom(backgroundColor: const Color(0xFFFF5B66), padding: const EdgeInsets.symmetric(vertical: 16)),
                          child: Text(analyzing ? 'Loading...' : 'Open link'),
                        ),
                      ),
                    ])),
                    if (error.isNotEmpty) ...[
                      ...
                      Container(
                        ...
                        child: Text(error, style: const TextStyle(color: Color(0xFFFFD6DA))),
                      ),
                    ],
                    const SizedBox(height: 16),
                    _panel(child: details == null ? _emptyState() : _detailsBody(details!)),
                  ],
                ),
        ),
      ),
  ...
  Widget _detailsBody(DxMediaDetails current) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Wrap(spacing: 8, runSpacing: 8, children: [
        ...
        if (current.duration != null) _chip(_formatDuration(current.duration!)),
        if (current.uploader.isNotEmpty) _chip(current.uploader),
      ]),
      const SizedBox(height: 14),
      Text(current.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
      if (current.description.isNotEmpty) ...[
        const SizedBox(height: 10),
        Text(current.description, style: const TextStyle(color: Color(0xFFD4CDD8))),
      ],
      const SizedBox(height: 16),
      ClipRRect(
        borderRadius: BorderRadius.circular(24),
        child: AspectRatio(
          aspectRatio: (current.previewWidth != null && current.previewHeight != null && current.previewHeight! > 0)
              ? current.previewWidth! / current.previewHeight!
              : 16 / 9,
          child: current.thumbnail.isNotEmpty
              ? Image.network(current.thumbnail, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _previewFallback())
              : _previewFallback(),
        ),
      ),
      const SizedBox(height: 8),
      if (current.previewUrl.isNotEmpty)
        const Text('Preview video URL is available from the backend. This mobile build shows the thumbnail and download options.', style: TextStyle(color: Color(0xFFC8BFD8))),
      const SizedBox(height: 16),
      Wrap(spacing: 8, runSpacing: 8, children: [
        if (current.uploadDate.isNotEmpty) _chip('Date ${current.uploadDate}'),
        if (current.viewCount != null) _chip('Views ${_formatCount(current.viewCount!)}'),
      ]),
      const SizedBox(height: 16),
      Row(children: [
        Expanded(child: ChoiceChip(label: const Text('Video'), selected: activeMode == DownloadMode.video, onSelected: (_) => setState(() => activeMode = DownloadMode.video))),
        const SizedBox(width: 12),
        Expanded(child: ChoiceChip(label: const Text('Audio'), selected: activeMode == DownloadMode.audio, onSelected: (_) => setState(() => activeMode = DownloadMode.audio))),
      ]),
      const SizedBox(height: 16),
      if (activeMode == DownloadMode.video)
        ..._videoTiles(current)
      else
        _downloadTile('MP3', 'Audio', downloadingKey == 'audio-mp3' ? 'Preparing...' : 'Download MP3', downloadingKey.isNotEmpty, () => _download(mode: DownloadMode.audio, key: 'audio-mp3')),
      if (latestDownload != null) ...[
        const SizedBox(height: 16),
        Container(
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: const Color(0x14FFFFFF), borderRadius: BorderRadius.circular(20)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Latest download', style: TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            Text(latestDownload!.path, style: const TextStyle(color: Color(0xFFD4CDD8))),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: OutlinedButton(onPressed: _openLatestDownload, child: const Text('Open file'))),
              const SizedBox(width: 12),
              Expanded(child: OutlinedButton(onPressed: _copyLatestPath, child: const Text('Copy path'))),
            ]),
          ]),
        ),
      ],
  ...
  List<Widget> _videoTiles(DxMediaDetails current) {
    final options = current.videoOptions.isEmpty ? [null] : current.videoOptions;
    return [
      for (var i = 0; i < options.length; i++) ...[
        _downloadTile(
          options[i]?.height == null ? 'Best quality' : '${options[i]!.height}p',
          options[i]?.label ?? 'Video',
          downloadingKey == (options[i] == null ? 'video-best' : 'video-${options[i]!.key}') ? 'Preparing...' : 'Download MP4',
  ...
  Widget _downloadTile(String title, String badge, String subtitle, bool disabled, VoidCallback onTap) {
    return InkWell(
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(999), color: const Color(0x22FF8F42)),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(subtitle, style: const TextStyle(color: Color(0xFFC8BFD8))),
            ])),
  ...
  Widget _panel({required Widget child}) {
    return DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0x22FFFFFF)),
        gradient: const LinearGradient(colors: [Color(0xD61B1625), Color(0xC9110E18)]),
        boxShadow: const [BoxShadow(color: Color(0x55000000), blurRadius: 24, offset: Offset(0, 16))],
      ),
  ...
  Widget _emptyState() => const SizedBox(
        ...
        child: Center(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Text('Ready', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
            SizedBox(height: 10),
            Text('Paste a supported link to load the preview and download choices.', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFFC8BFD8))),
          ]),
        ),
      );
  ...
  Widget _previewFallback() => const ColoredBox(
        ...
        child: Center(child: Text('No preview', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700))),
      );
  ...
  String _normalizeBaseUrl(String input) {
    ...
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) throw const DxException('Enter a valid backend URL.');
    return value.replaceFirst(RegExp(r'/+$'), '');
  }

  Uri _apiUri(String baseUrl, String endpoint, [Map<String, String>? query]) {
    final base = Uri.parse(baseUrl);
    final path = '${base.path.endsWith('/') ? base.path.substring(0, base.path.length - 1) : base.path}${endpoint.startsWith('/') ? endpoint : '/$endpoint'}';
  ...
  String _friendlyError(Object err) {
    ...
    if (err is DioException) {
      ...
      if (err.type == DioExceptionType.connectionError || err.type == DioExceptionType.connectionTimeout || err.type == DioExceptionType.receiveTimeout) {
        return 'DX could not reach the server. Check the backend URL and try again.';
      }
    }
    return 'Something went wrong.';
  }

  String _fileNameFromHeaders(String? contentDisposition, String title, DownloadMode mode) {
    if (contentDisposition != null) {
      final utf = RegExp(r"filename\*=UTF-8''([^;]+)", caseSensitive: false).firstMatch(contentDisposition);
  ...
  String _formatDuration(int seconds) {
    ...
    return hours > 0 ? '$hours:${minutes.toString().padLeft(2, '0')}:${remaining.toString().padLeft(2, '0')}' : '$minutes:${remaining.toString().padLeft(2, '0')}';
  }
...
class DxVideoOption {
  DxVideoOption({required this.key, required this.label, required this.quality, required this.height});
...
String _string(Object? value, {String fallback = ''}) => value is String && value.trim().isNotEmpty ? value.trim() : fallback;
